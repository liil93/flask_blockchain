...
def Make_db():
    ...
    cursor = con.cursor()
    cursor.execute("CREATE TABLE member(Email text NOT NULL UNIQUE, PW text NOT NULL,PN text DEFAULT 0, CC text DEFAULT 0, AP text DEFAULT 0)")
    con.commit()
    con.close()
...
def Make_db_pet():
    ...
    cursor = con.cursor()
    cursor.execute("CREATE TABLE pet(Host text NOT NULL,P_key text, Name text, Birth text, Gender text, Kind text,NS text, Vac text, PRIMARY KEY(Host), CONSTRAINT fk_PerPet FOREIGN KEY (Host) REFERENCES member(Email))")
    con.commit()
    con.close()
...
def Make_db_house():
    ...
    cursor = con.cursor()
    cursor.execute("CREATE TABLE house(Host text NOT NULL, Address text, Type text, Room text, Area text, Elevator text, Parking text, PRIMARY KEY(Host), CONSTRAINT fk_PerPet FOREIGN KEY (Host) REFERENCES member(Email))")
    con.commit()
    con.close()
